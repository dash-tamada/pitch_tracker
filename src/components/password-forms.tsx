...
export function SetPasswordForm() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try { await api("POST", "/api/v1/auth/password/reset", { token, password: f.get("password") }); setDone(true); }
    catch (err) { setError(err instanceof Error ? err.message : "Failed"); }
  }
  if (token === null) return null;
  if (!token) return <form><h1>Link not valid</h1><p>Ask your administrator for a new link.</p></form>;
