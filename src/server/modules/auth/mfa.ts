...
export async function beginEnrolment(db: Db, userId: string, ctx: RequestContext = {}) {
  ...
  await writeAudit(db, { actorId: userId, action: "auth.mfa_enrolment_started", resourceType: "user", resourceId: userId }, ctx);
...
export async function confirmEnrolment(db: Db, userId: string, sessionId: string, raw: unknown, ctx: RequestContext = {}, now = Date.now()) {
  ...
  await db.transaction(async (tx) => {
    ...
    await writeAudit(tx, { actorId: userId, action: "auth.mfa_enabled", resourceType: "user", resourceId: userId }, ctx);
  });
}
...
export async function verifySessionMfa(db: Db, userId: string, sessionId: string, raw: unknown, ctx: RequestContext = {}, now = Date.now()) {
  ...
  if (step === null) {
    await writeAudit(db, { actorId: userId, action: "auth.mfa_failed", resourceType: "user", resourceId: userId }, ctx);
  ...
  await db.transaction(async (tx) => {
    ...
    await writeAudit(tx, { actorId: userId, action: "auth.mfa_verified", resourceType: "user", resourceId: userId }, ctx);
  });
}
